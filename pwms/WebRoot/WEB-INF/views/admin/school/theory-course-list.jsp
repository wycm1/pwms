...
<div class="pd-20">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a class="btn btn-primary radius" onclick="article_add('添加理论课程','theory-course-add.html')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加理论课程</a></span> <span class="r">共有数据：<strong><c:out value="${fn:length(tCourseList)}"></c:out></strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<c:forEach items="${tCourseList}" var="item">
				<tr class="text-c">
					...
					<td>${item.id}</td>
					<td><a href="/pwms/school/llkc/${item.id}/course.html" target="_blank">${item.title}</a></td>
					<td>admin</td>
					<td>${item.dateline}</td>
					<td>${item.visits}</td>
